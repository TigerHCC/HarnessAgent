...
.DESCRIPTION
  ...
  The 18 servers (all loopback, streamable HTTP). The first 12 are read-only diagnostic MCPs:
  ...
  read tools). audio 8796 is the Windows Audio Diagnostics MCP -- read-only hardware/codec/mic/Bluetooth
  diagnosis (RunLevel Highest). obsidian, dtm_download, and scheduler are the only three servers that run
  UNELEVATED (RunLevel Limited).
...
if ($manifestEntries.Count -ne 18) { Die "MCP manifest must contain exactly 18 entries on canonical ports 8777-8793 + 8796; found $($manifestEntries.Count) entries." }
...
foreach ($entry in $manifestEntries) {
  ...
  if ($entry.port -notin $expectedPorts) {
    Die "MCP manifest must use canonical ports 8777-8793 + 8796 exactly once."
  }
...
Write-Host "=== HarnessAgent MCP servers $mode (18: 12 diagnostic + dtmsdk + obsidian + dtm_download + dtm_deploy + scheduler + audio) ===" -ForegroundColor Magenta
