# Adds the docstruct extension to goose's config.yaml (idempotent; backs up first).
...
if ($cfg -match "(?m)^\s{2}docstruct\s*:") { Write-Host "[OK] docstruct already present -- no change." -ForegroundColor Green; exit 0 }
Copy-Item $ConfigPath "$ConfigPath.bak-docstruct" -Force
$block = @"
  ...
  docstruct:
    ...
    env_keys: []
    timeout: 120
    description: 'Structured document extraction: PDF text-layer/OCR (RapidOCR) to text, and schema/template-driven field->value JSON via the local vLLM (docstruct MCP, manifest-external, 127.0.0.1:8795).'
"@
...
if ($py) {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        if ($LASTEXITCODE -ne 0) {
            Copy-Item "$ConfigPath.bak-docstruct" $ConfigPath -Force
            Write-Host "[X] resulting YAML failed to parse -- restored from backup, no change applied." -ForegroundColor Red
            exit 1
        }
    }
}
Write-Host "[OK] Added docstruct extension to $ConfigPath (backup: $ConfigPath.bak-docstruct)" -ForegroundColor Green
